...
public class LC1298MaximumCandiesYouCanGetFromBoxes {
    ...
    public int maxCandies(int[] status, int[] candies, int[][] keys, int[][] containedBoxes, int[] initialBoxes) {
        ...
        boolean[] seenBox = new boolean[len];
        boolean[] hasKey = new boolean[len];
        ...
        while (!queue.isEmpty()) {
            ...
            // 队列中的box一定是有key的 所以可以直接打开
            result += candies[curBox];

            /*
            从队列中取出一个box 后需要做下面几个事情
            1. 更新result(因为队列中的box必须都有Key)
            2. 更新seenBox(当前box的嵌套box集合)
            3. 更新hasKey(当前box里嵌套的key集合)
            4. 更新队列 看到过的box且这次拿到新的key 就入队
             */
            for (int containedBox: containedBoxes[curBox]) {
                seenBox[containedBox] = true;
                if (hasKey[containedBox]) {
                    queue.offer(containedBox);
                }
            }

            // 用当前box里面能拿到的钥匙 更新队列和hasKey[]
            for (int box : keys[curBox]) {
                // 之前没有这个box的钥匙且我们看到过这个box->要防止死循环 这里可能会重复入队
                if (!hasKey[box] && seenBox[box]) {
                    queue.offer(box);
                }
                // 不管怎样 得到了新钥匙
                hasKey[box] = true;
